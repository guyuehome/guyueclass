...
// C/C++ source code generated on : Sun Mar 27 13:28:55 2022
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"PID_identification\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(PID_identification_M, (NULL));
        PID_identification_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(PID_identification_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(PID_identification_M)
                == (NULL)) || rtmGetStopRequested(PID_identification_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(PID_identification_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
