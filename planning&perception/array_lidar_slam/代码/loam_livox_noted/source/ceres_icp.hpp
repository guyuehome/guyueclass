...
//point-to-line with motion deblur(线性插值)
template <typename _T>
struct ceres_icp_point2line_mb {
    ...
    _T m_motion_blur_s;
    Eigen::Matrix<_T, 4, 1> m_q_last; //实部在前，上一帧end在map下位姿
    Eigen::Matrix<_T, 3, 1> m_t_last;
    _T m_weigh;
    ceres_icp_point2line_mb(const Eigen::Matrix<_T, 3, 1>& current_pt,                         //在当前帧下的corner点，见paper LOAM: Lidar Odometry and Mapping in Real-time
                            const Eigen::Matrix<_T, 3, 1>& target_line_a,                      //local map corner(j)
                            const Eigen::Matrix<_T, 3, 1>& target_line_b,                      //local map corner(l)
                            const _T motion_blur_s = 1.0,                                      //线性插值的比例
                            Eigen::Matrix<_T, 4, 1> q_s = Eigen::Matrix<_T, 4, 1>(1, 0, 0, 0), //m_q_w_last
                            Eigen::Matrix<_T, 3, 1> t_s = Eigen::Matrix<_T, 3, 1>(0, 0, 0))    //m_t_w_last
                            : m_current_pt(current_pt), m_target_line_a(target_line_a),
    ...
    bool operator()(const T* _q, const T* _t, T* residual) const {
        ...
        Eigen::Quaternion<T> q_incre{_q[3], _q[0], _q[1], _q[2]}; //_q: 实部在后
        Eigen::Matrix<T, 3, 1> t_incre{_t[0], _t[1], _t[2]};
        ...
        Eigen::Matrix<T, 3, 1> pt_transfromed; //i point
        pt_transfromed = q_last * (q_interpolate * pt + t_interpolate) + t_last; //先转换到当前帧start下(上一帧end)，然后转化到map下
        ...
        Eigen::Matrix<T, 3, 1> vec_ac = pt_transfromed - tar_line_pt_a; //i-j
        Eigen::Matrix<T, 3, 1> residual_vec = vec_ac - Eigen_math::vector_project_on_unit_vector(vec_ac, vec_line_ab_unit); //点到直线的距离向量，由垂足指向点
    ...
    static ceres::CostFunction* Create(const Eigen::Matrix<_T, 3, 1>& current_pt,
                                       ...
                                       const _T motion_blur_s = 1.0,
                                       Eigen::Matrix<_T, 4, 1> q_last = Eigen::Matrix<_T, 4, 1>(1, 0, 0, 0), //实部在前
...
// point to plane with motion deblur(线性插值)
template <typename _T>
...
//point-to-line （分段） loop closure时使用
template <typename _T>
...
// point to plane（分段）loop closure时使用
template <typename _T>
