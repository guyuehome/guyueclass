...
class Scene_alignment {
    ...
    static int load_pose_and_regerror(std::string file_name,
     ...
     //计算correct_curr_to_history 约束
    static Ceres_pose_graph_3d::Constraint3d add_constrain_of_loop
                (int s_idx,                                      //curr_finished_keyframe_index
                int t_idx,                                       //history_finished_keyframe_index
                Eigen::Quaterniond q_a, Eigen::Vector3d t_a,     //history_q,     history_t
                Eigen::Quaterniond q_b, Eigen::Vector3d t_b,     //wrong_curr_q,  wrong_curr_t
                Eigen::Quaterniond icp_q, Eigen::Vector3d icp_t, //his2Wrongcurr_q_in_map, his2Wrongcurr_t_in_map
                int if_verbose = 1) {                            //1
   ...
   //把每个finished keyframe的poses（histories, wrong_curr）, 位姿之间的正确约束(histories, correct_curr2history)写入到g2o文件中
    ...
    void set_downsample_resolution(const float& line_res, const float& plane_res) { //laser_mapping.hpp中从yaml获取 map_alignment_resolution = 0.1
        m_line_res = line_res;   //0.1
        m_plane_res = plane_res; //0.1
    ...
    //called by laser_mapping.hpp when loop closure 
    void init(std::string path) {
        ...
        //(第1处)设置registration params在laser_mapping.hpp func service_loop_detection()中
        //(第2处)设置registration params
        m_pc_reg.ICP_LINE = 0; //loop closure 时为 0,不用corners 来对齐
        ...
        m_pc_reg.m_max_final_cost = 20000; //not useful, loop closure时:  没有通过函数的返回值是0 or 1 来判断是否对齐, 而是通过m_inlier_threshold来判断

        m_pc_reg.m_para_max_speed = 2.0;  //TODO  in point_cloud_registration.hpp set_ceres_solver_bound() 设置deleta_trans下界和上界, default: 1000.0

        m_pc_reg.m_para_max_angular_rate = 360; //not useful, loop closure时:  没有通过函数的返回值是0 or 1 来判断是否对齐, 而是通过m_inlier_threshold来判断
        //TODO 应该为360度, default: 360 * 57.3

        // m_pc_reg.m_para_icp_max_iterations = 10;  后面还会设置,此处不起作用
        // m_pc_reg.m_para_cere_max_iterations = 20; 后面还会设置,此处不起作用
    ...
    int find_tranfrom_of_two_mappings(Maps_keyframe<PT_DATA_TYPE>* keyframe_a, //当前finished keyframe
                                      Maps_keyframe<PT_DATA_TYPE>* keyframe_b, //正在比较的history keyframe
                                      int if_save = 1,                         //0
                                      std::string mapping_save_path = std::string(" ")) {
        ...
        //(第3处)设置registration params
        m_pc_reg.m_current_frame_index = 10000000;
        m_pc_reg.m_q_w_curr.setIdentity(); //curr_q = I

        m_pc_reg.m_q_w_last.setIdentity();
        m_pc_reg.m_t_w_last.setZero(); //last 设置为(I, 0)

        m_pc_reg.m_para_icp_max_iterations = m_maximum_icp_iteration; //yaml 设置 5
        m_pc_reg.m_para_cere_max_iterations = 200; //default 50
        m_pc_reg.m_para_cere_prerun_times = 2;
        m_pc_reg.m_maximum_allow_residual_block = m_para_scene_alignments_maximum_residual_block; //3000
        m_pc_reg.m_if_verbose_screen_printf = m_if_verbose_screen_printf; //1

        // Eigen::Matrix<double, 3, 1> transform_T = (keyframe_a->get_center() - keyframe_b->get_center()).template cast<double>(); default
        Eigen::Matrix<double, 3, 1> transform_T = (keyframe_b->get_center() - keyframe_a->get_center()).template cast<double>();
        //TODO 应该是 b - a， 作者是把history点转换的 
        //keyframe_a and keyframe_b centers are all in map,  a-b is a vector in map from b point to a
        //the points stored in a and b frame are all in map frame, so points_stored_inB + (a-b) is points_stored_inA
        
        Eigen::Quaterniond transform_R = Eigen::Quaterniond::Identity(); //not used
        m_pc_reg.m_t_w_incre = transform_T; //incre_t = T, incre_q = I
        m_pc_reg.m_t_w_curr = transform_T; //curr_t = T 


        // Common_tools::Timer timer;
        // timer.tic("Total");
        for (int scale = 8; scale >= 0; scale -= 4) { //循环3次，scale=8, 4, 0 不同降采样分辨率下对齐
            // timer.tic("Each omp");

            float line_res = m_line_res * scale;
            float plane_res = m_plane_res * scale;
            if (line_res < m_line_res) {//最后一次没有放大分辨率
                line_res = m_line_res;
            }

            if (plane_res < m_plane_res) {//false
            ...
            m_pc_reg.find_out_incremental_transfrom(sourece_pt_line_ds.makeShared(), //当前finished keyframe
                                                    sourece_pt_plane_ds.makeShared(),
                                                    target_pt_line_ds.makeShared(), //正在比较的history keyframe
                                                    target_pt_plane_ds.makeShared());
            //执行两帧点云对齐  从curr到history(transform points in history into curr)?
            ...
            screen_out << "Q_curr is: " << m_pc_reg.m_q_w_curr.coeffs().transpose() << endl;  //loop closure时, curr = incre
            screen_out << "T_curr is: " << m_pc_reg.m_t_w_curr.transpose() << endl;
            // screen_out << timer.toc_string("Each omp") << std::endl;
            
            //TODO why break? we comment this to try
            // if (m_pc_reg.m_inlier_threshold > m_accepted_threshold * 2) //0.4, set from yaml
            //     break;
        }
