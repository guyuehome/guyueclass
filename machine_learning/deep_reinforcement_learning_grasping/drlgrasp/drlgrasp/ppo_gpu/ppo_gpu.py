...
#from drlgrasp.ppo.logx import EpochLogger
...
def ppo(env_fn,
    ...
    ac_pi = actor(obs_dim[0], act_dim[0])
    ac_v = critic(obs_dim[0])
    ...
    def update():
        ...
        for i in range(train_pi_iters):
            ...
            mpi_avg_grads(ac_pi.cpu())  # average grads across MPI processes
            ac_pi.to(device)
            pi_optimizer.step()
        ...
        for i in range(train_v_iters):
            ...
            loss_v.backward()
            mpi_avg_grads(ac_v.cpu())  # average grads across MPI processes
            ac_v.to(device)
            vf_optimizer.step()
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            with torch.no_grad():
                o_tensor = torch.as_tensor(o.copy(), dtype=torch.float32).to(device)
                pi, _ = ac_pi(o_tensor)
                a = pi.sample()
                logp = pi.log_prob(a).sum(axis=-1)
                v = ac_v(torch.as_tensor(o,dtype=torch.float32).to(device))
                #print(f'a={a},logp_a={logp},v={v},pi={pi}')
            ...
            if terminal or epoch_ended:
                ...
                if timeout or epoch_ended:
                    with torch.no_grad():
                        v = ac_v(torch.as_tensor(o.copy(), dtype=torch.float32).to(device)).cpu().numpy()
                else:
                    v = 0
                ...
                if terminal:
                    ...
                    print(Fore.GREEN +
                          'ep_ret={}, ep_len={}'.format(ep_ret, ep_len))
                o, ep_ret, ep_len = env.reset(), 0, 0
