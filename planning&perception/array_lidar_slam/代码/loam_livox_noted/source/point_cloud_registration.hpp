...
class Point_cloud_registration {
    ...
    double m_para_buffer_RT[7] = {0, 0, 0, 1, 0, 0, 0};      //not useful
    double m_para_buffer_RT_last[7] = {0, 0, 0, 1, 0, 0, 0}; //not useful
    ...
    int m_if_motion_deblur = 0; //motion deblur(去模糊), 0: piesewise processing;  1: linear interpolation； set by yaml
                                //loop clousre(scene_alignment.hpp)时： 0 
    ...
    Eigen::Quaterniond m_q_w_curr, m_q_w_last; //在构造函数中初始化为I, zero
 ...
 */
    ...
    void set_ceres_solver_bound(ceres::Problem& problem, double* para_buffer_RT) {
        for (unsigned int i = 0; i < 3; i++) {
            problem.SetParameterLowerBound(para_buffer_RT + 4, i, -m_para_max_speed); //-2
            problem.SetParameterUpperBound(para_buffer_RT + 4, i, +m_para_max_speed); //2
        }
    }
   ...
   //<2>loop clousre(scene_alignment.hpp)，计算的是history --->curr(TF in map)
    int find_out_incremental_transfrom(pcl::PointCloud<PointType>::Ptr in_laser_cloud_corner_from_map,       //<1>:local map corners          <2>curr finished keyframe corners(points in map)
        ...
        if (laserCloudCornerFromMapNum > CORNER_MIN_MAP_NUM && 
            ...
            m_current_frame_index > m_mapping_init_accumulate_frames) {//在laser_mapping.hpp中通过yaml设置为50
            ...
            for (iterCount = 0; iterCount < m_para_icp_max_iterations; iterCount++) { //计算位姿时, yaml: 15; loop closure, yaml: 5
                ...
                for (int i = 0; i < laser_corner_pt_num; i++) { //curr帧corners
                    ...
                    pointAssociateToMap(&pointOri, &pointSel, 
                                        refine_blur(pointOri.intensity, m_minimum_pt_time_stamp, m_maximum_pt_time_stamp), //<2>时：返回1
                                        if_undistore_in_matching); //1
                    ...
                    if (m_point_search_sq_dis[line_search_num - 1] < m_maximum_dis_line_for_match) {//2m
                        bool line_is_avail = true;
                        std::vector<Eigen::Vector3d> nearCorners;
                        Eigen::Vector3d center(0, 0, 0);
                        if (IF_LINE_FEATURE_CHECK) {//0
            ...
            *(m_logger_common->get_ostream()) << "Iteration time: " << iterCount << endl<<endl;
            ...
            if (m_angular_diff > m_para_max_angular_rate || minimize_cost > m_max_final_cost) {
                ...
                *(m_logger_common->get_ostream()) << summary.FullReport() << endl;
                for (int i = 0; i < 7; i++) {
                    m_para_buffer_RT[i] = m_para_buffer_RT_last[i]; //其他地方没有使用这两个变量， not useful
                }
    ...
    int find_out_incremental_transfrom(pcl::PointCloud<PointType>::Ptr in_laser_cloud_corner_from_map, //curr finished keyframe
        ...
        return find_out_incremental_transfrom(in_laser_cloud_corner_from_map, in_laser_cloud_surf_from_map, //curr finished keyframe
                                              m_kdtree_corner_from_map, m_kdtree_surf_from_map, //curr finished frame kdtree
                                              laserCloudCornerStack, laserCloudSurfStack); //history finished keyframe
    }
    ...
    void pointAssociateToMap(PointType const* const pi, PointType* const po,
        ...
        if (m_if_motion_deblur == 0 || if_undistore == 0 || interpolate_s == 1.0) {
            ...
            if (1) // Using rodrigues for fast compute.
            {
                ...
                interpolate_R_mat = Eigen::Matrix3d::Identity() + sin(interpolate_R_theta) * m_interpolatation_omega_hat 
                ...
                Eigen::Vector3d interpolate_T = m_t_w_incre * (interpolate_s * BLUR_SCALE);
                //(interpolate_q * point_curr + interpolate_T)： 先转换到上一帧end下
    ...
    unsigned int pointcloudAssociateToMap(pcl::PointCloud<PointType> const& pc_in, pcl::PointCloud<PointType>& pt_out, int if_undistore = 0) {
        ...
        for (unsigned int i = 0; i < points_size; i++) {
            pointAssociateToMap(&pc_in.points[i], &pt_out.points[i],
                                refine_blur(pc_in.points[i].intensity, m_minimum_pt_time_stamp, m_maximum_pt_time_stamp),
                                if_undistore);
        }
