...
Px=0.15*sin(t);     Py=0.13*sin(t);    Pz=-0.05*sin(t);
Pdx=0.15*cos(t);    Pdy=0.13*cos(t);   Pdz=-0.05*cos(t);
Pddx=-0.15*sin(t);   Pddy=-0.13*sin(t);  Pddz=0.05*sin(t);

show_vel = [Pdx Pdy Pdz]
show_acc = [Pddx Pddy Pddz]
...
epsilon00=[0,0,0]';
epsilon11=transpose(R{1})*(epsilon00+cross(omega00,[0,0,1]'*qd1)+[0,0,1]'*qdd1);
epsilon22=transpose(R{2})*(epsilon11+cross(omega11,[0,0,1]'*qd2)+[0,0,1]'*qdd2);
epsilon33=transpose(R{3})*(epsilon22+cross(omega22,[0,0,1]'*qd3)+[0,0,1]'*qdd3);
a00=[0,0,0]';
