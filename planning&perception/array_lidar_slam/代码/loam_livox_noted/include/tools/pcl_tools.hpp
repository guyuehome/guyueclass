...
namespace PCL_TOOLS {
    ...
    struct Pt_compare {
        ...
        template <typename _T> //Eigen::Matrix<float, 3, 1>
    ...
    struct Eigen_pt_hasher {
        template <typename _T> //Eigen::Matrix<float, 3, 1>
        std::size_t operator()(const _T& k) const { //自定义的hash对象，用到了std::hash
    ...
    pcl::PointCloud<PointType> pointcloud_transfrom(pcl::PointCloud<PointType> pcl_pt_in,
                             Eigen::Matrix<T, 3, 3> tranfrom_R, Eigen::Matrix<T, 3, 1> tranfrom_T) {
    ...
    void hsv2rgb(const unsigned char& src_h, const unsigned char& src_s, const unsigned char& src_v, 
