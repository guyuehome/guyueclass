...
{
    double m_first_imu_time = -3e88; // 第一帧IMU时间
    double m_sliding_window_tim = 10000; // 暂时相当于没用
    double m_last_imu_time = -3e88; // 上一帧IMU的时间
    double m_last_visual_time = -3e88; // img_pose->m_timestamp + g_lio_state.td_ext_i2c;
    ...
    int m_if_acc_mul_G = 0;  // 量纲从G转化成m/s^2

    int m_if_have_lidar_data = 0;  // 第一帧lidar_callback后等于1
    int m_if_have_camera_data = 0; // 第一帧camera_callback后等于1
    ...
    std::deque<sensor_msgs::PointCloud2::ConstPtr> *m_liar_frame_buf = nullptr;//保存lidar的msg的队列
