...
private:
    ...
    bool termination_flag = false;                                                                              // 用于终止多线程重建树的标志
    bool rebuild_flag = false;                                                                                  // 用于标志是否需要重建树的标志
    pthread_t rebuild_thread;                                                                                   // 用于多线程重建树的线程
    ...
    int Treesize_tmp = 0, Validnum_tmp = 0;         // 用于多线程重建树的临时变量
    float alpha_bal_tmp = 0.5, alpha_del_tmp = 0.0; // 用于多线程重建树的临时变量
    float delete_criterion_param = 0.5f;            // 用于多线程重建树的临时变量
    float balance_criterion_param = 0.7f;           // 用于多线程重建树的临时变量
    float downsample_size = 0.2f;                   // 用于多线程重建树的临时变量
    bool Delete_Storage_Disabled = false;
    KD_TREE_NODE *STATIC_ROOT_NODE = nullptr;
    PointVector Points_deleted;             // 用于多线程重建树的点云存储
    PointVector Downsample_Storage;         // 用于多线程重建树的点云存储
    PointVector Multithread_Points_deleted; // 用于多线程重建树的点云存储
...
public:
    ...
    KD_TREE_NODE *Root_Node = nullptr; //设置根节点一开始为空
    int max_queue_size = 0;            //设置最大队列长度为0
};
